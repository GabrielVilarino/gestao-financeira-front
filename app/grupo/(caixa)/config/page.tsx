...
export default function GrupoConfigPage() {
  ...
  const [categoriaEditData, setCategoriaEditData] = useState<
    CategoriaFormData | undefined
  >()
  ...
  const [subCategoriaEditData, setSubCategoriaEditData] = useState<
    SubCategoriaFormData | undefined
  >()
  const [subCategoriaConfirmOpen, setSubCategoriaConfirmOpen] = useState(false)
  const [subCategoriaDeleteId, setSubCategoriaDeleteId] = useState<
    number | undefined
  >()
  ...
  async function fetchCategorias(): Promise<Categoria[]> {
    try {
      const response = await fetch("/api/categorias?group=true")
  ...
  function handleOpenEditCategoria(id: number) {
    ...
    setCategoriaEditData({
      nome: categoria.nome,
      tipo_movimentacao: categoria.tipo_movimentacao as "receita" | "despesa",
    })
    setCategoriaDialogOpen(true)
  }
  ...
  async function handleConfirmDeleteCategoria() {
    ...
    const response = await fetch(
      `/api/categorias/delete/${categoriaDeleteId}?group=true`,
      { method: "DELETE" }
    )
  ...
  async function handleSubmitCategoria(data: CategoriaFormData, id?: number) {
    ...
    const url = isEditing
      ? "/api/categorias/update?group=true"
      : "/api/categorias/create?group=true"
  ...
  async function fetchSubCategorias(cats: Categoria[]): Promise<SubCategoria[]> {
    try {
      ...
      return data.map((sub) => ({
        ...sub,
        nome_categoria:
          cats.find((c) => c.id_categoria === sub.id_categoria)?.nome ?? "",
      }))
    } catch (error) {
  ...
  async function handleConfirmDeleteSubCategoria() {
    ...
    const response = await fetch(
      `/api/subcategorias/delete/${subCategoriaDeleteId}?group=true`,
      { method: "DELETE" }
    )
  ...
  async function handleSubmitSubCategoria(data: SubCategoriaFormData, id?: number) {
    const isEditing = id !== undefined
    const url = isEditing
      ? "/api/subcategorias/update?group=true"
      : "/api/subcategorias/create?group=true"
    const method = isEditing ? "PUT" : "POST"
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col gap-4 p-4 pt-0 overflow-y-auto min-h-0">
          <div className="flex flex-col">
            <h1>
              <b>Configurações de Categorias</b>
            </h1>
            <DataTable
          ...
          <div className="flex flex-col">
            <h1>
              <b>Configurações de SubCategorias</b>
            </h1>
            <DataTable
